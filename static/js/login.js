...
$(function() {
    ...
    function send_sms_code(sendBtn,tipsId){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $sendBtn.val("Send verification code");
                if(data.mobile){
                    ...
                    Dml.fun.showErrorTips($tipsId, "SMS verification code has been sent");
                    $sendBtn.attr("disabled","disabled");
                    // show_send_sms(60);
                    daojishi(60, '#jsSendCode')
                }
    ...
    function show_send_sms(time){
        $('#jsSendCode').val("Resend in " + seconds + "seconds");
        if(time<=0){
            clearTimeout(send_sms_time);
            $('#jsMobileTips').hide(500);
             $('#jsSendCode').val("Send verification code").removeAttr("disabled");
            return;
        }
        time--;
    ...
    function daojishi(seconds,obj){
        if (seconds > 1){
                seconds--;
                $('#jsSendCode').val("Resend in " + seconds + "seconds").attr("disabled", true);
